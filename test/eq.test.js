import { expect } from 'chai';
import eq from '../src/eq.js';

describe('Test suite 5: Testing eq utility: Tests generated with Microsoft Copilot AI (Tuni)', () => {
  it('TC1: Should return true for the same object reference', () => {
    const object = { a: 1 };
    expect(eq(object, object)).to.be.true;
  });

  it('TC2: Should return false for different objects with same properties', () => {
    const object = { a: 1 };
    const other = { a: 1 };
    expect(eq(object, other)).to.be.false;
  });

  it('TC3: Should return true for equal primitive strings', () => {
    expect(eq('a', 'a')).to.be.true;
  });

  it('TC4: Should return false for string and String object', () => {
    expect(eq('a', Object('a'))).to.be.false;
    // Should return false, but does return true.
  });

  it('TC5: Should return true for NaN compared to NaN', () => {
    expect(eq(NaN, NaN)).to.be.true;
  });

  it('TC6: Should return true for equal numbers', () => {
    expect(eq(42, 42)).to.be.true;
  });

  it('TC7: Should return false for different numbers', () => {
    expect(eq(42, 43)).to.be.false;
  });

  it('TC8: Should return true for boolean values that are equal', () => {
    expect(eq(true, true)).to.be.true;
    expect(eq(false, false)).to.be.true;
  });

  it('TC9: Should return false for different boolean values', () => {
    expect(eq(true, false)).to.be.false;
  });

  it('TC10: Should return true for null compared to null', () => {
    expect(eq(null, null)).to.be.true;
  });

  it('TC11: Should return false for null compared to undefined', () => {
    expect(eq(null, undefined)).to.be.false;
    // This also returns true, even though it should return false when using strict equality.
  });

  it('TC12: Should return true for undefined compared to undefined', () => {
    expect(eq(undefined, undefined)).to.be.true;
  });

  it('TC13: Should return false for number and string with same value', () => {
    expect(eq(1, '1')).to.be.false;
    // Returns true, but it should return false in strict sense.
  });
});